...
```
ssh-keygen -t rsa -f provision/hana_node/files/sshkeys/cluster.id_rsa
```
...
* The registration code if you want to register your system, mandatory to download salt-minion if it isn't include in the image.
* The deployment mode, if set to true, it disables all extra packages that do not come from the image.
 ...
 * **regcode**: Needed if you want to register your system and mandatory to download salt-minion if it isn't include in the image.
 * **testmode**: The deployment mode, if set to true, it disables all extra packages that do not come from the image (for example, we use `true`to perform the Build Validation of a new AWS Public Cloud image)
 * **hosts_ips**: Each cluster nodes IP address (sequential order). Mandatory to have a generic `/etc/hosts` file.
 ...
 To customize this provisioning, you can change the configuration into the pillar files (cluster.sls and hana.sls) located in provision/hana_node/files/pillar/.
 For example, you will find the files used for Build validation in provision/hana_node/files/pillar/QA_templates/.
